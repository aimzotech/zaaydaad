<?php
function miniorange_otp_feedback()
{
  If ((isset($_POST['mo_otp_check'])) && ($_POST['mo_otp_check'] == "True")) {
    ...
    $email = !empty(variable_get('miniorange_otp_customer_admin_email', '')) ? variable_get('miniorange_otp_customer_admin_email', '') : $_POST['miniorange_feedback_email'];

    $phone = variable_get('miniorange_otp_customer_admin_phone', '');
    $customerKey = variable_get('miniorange_otp_customer_id', '');
    ...
    $currentTimeInMillis = get_otp_timestamp();
    ...
    $fromEmail = $email;
    $subject = "Drupal 7 OTP Verification Module Feedback";

    $query = '[Drupal 7 OTP]: ' . $message;
          ...
          <div class="modal-content" style="border-radius: 20px;">
            <div class="modal-header"
              ...
              <h4 class="modal-title" style="color: white; text-align: center;">
                Hey, it seems like you want to deactivate miniOrange OTP Verification Module
              </h4>
              <hr>
            ...
            <div class="modal-body"
              ...
              <form name="f" method="post" action="" id="mo_feedback">
                <div>
                  <p>
                    <?php if (empty(variable_get('miniorange_otp_customer_admin_email', ''))) { ?>
...
function hiddenOtpFields($key, $value)
{
  $hiddenOtpField = "";
  $value2 = array();
  if (is_array($value)) {
    foreach ($value as $key2 => $value2) {
      if (is_array($value2)) {
        hiddenOtpFields($key . "[" . $key2 . "]", $value2);
      } else {
...
//    $currentTimeInMillis = round(microtime(true) * 1000);
    $currentTimeInMillis = MiniorangeOtpCustomer::get_oauth_timestamp();
